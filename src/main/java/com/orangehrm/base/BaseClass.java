...
//	 private static ActionDriver actionDriver;
	// new changes as per Thread local Parallel TESTING
	...
	public void loadConfig() throws IOException {
		...
		// Start the Extent Report---Already implemented in ITestListner
//		 ExtentManager.getReporter();//--This has been implemented in ITestListner
//	public void onStart(ITestContext context)Triggered when a suite Starts

	}

	@BeforeMethod
	public synchronized void setup() throws IOException { // to run thread local Parallel='methods"
...
//		 Initialize ActionDriver for the current Thread
	 ...
	 */
	private synchronized void launchBrowser() { // for thread local
...
//		private  void launchBrowser() {
		...
		if (browser.equalsIgnoreCase("chrome")) {
			...
			// C:\Users\Akanksha\AppData\Local\Programs\Python\Python38\Scripts\chromedriver.exe
...
//			 driver = new ChromeDriver();
			driver.set(new ChromeDriver(options)); // New Changes as per Thread // New Changes as per Thread
...
//		driver.manage().window().maximize();
		// Navigate to URL
		try {
			getDriver().get(prop.getProperty("url")); // new changes as per Thread local Parallel TESTING
//			driver.get(prop.getProperty("url"));
	...
	// Driver setter method//new changes as per Thread local Parallel TESTING
	...
	// Getter Method for ActionDriver//new changes as per Thread local Parallel
	// TESTING
...
//	public  void tearDown() {
	public synchronized void tearDown() { // for thread local
		if (driver.get() != null) {
			try {
				driver.get().quit();
			} catch (Exception e) {
...
//		 driver = null;
//		 actionDriver = null;
//		ExtentManager.endTest(); // End test to flush the report--This hasbeen implemented in ITESTListner
	}
